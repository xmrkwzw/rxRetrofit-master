...
public class Camera2Activity extends BaseActivity {
	...
	@OnClick({R.id.surfaceview, R.id.iv_take_photo})
		...
		mImageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() { //可以在这里处理拍照得到的临时照片 例如，写入本地
			...
			public void onImageAvailable(ImageReader reader) {
				...
				if (bitmap != null) {
					...
					intent.putExtra(Constant.Intent_key.URI, CommonUtils.bitmap2Uri(getApplicationContext(),bitmap));
	...
	private CameraDevice.StateCallback stateCallback = new CameraDevice.StateCallback() {
		...
		public void onError(CameraDevice camera, int error) {//发生错误
			ToastUtils.showShortMes(Camera2Activity.this, "摄像头开启失败");
		}
	};
		...
		try {
			...
			mCameraDevice.createCaptureSession(Arrays.asList(mSurfaceHolder.getSurface(), mImageReader.getSurface()), new CameraCaptureSession.StateCallback() // ③
			{
				...
				public void onConfigureFailed(CameraCaptureSession cameraCaptureSession) {
					ToastUtils.showShortMes(Camera2Activity.this, "配置失败");
				}
			}, childHandler);
		} catch (CameraAccessException e) {
			e.printStackTrace();
